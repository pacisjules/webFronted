...
import { getpermissions} from "../../../features/userdetails/userdetails.js";
...
import { UPDATEdetails, DeleteDetail, AddDetail } from "../../../features/userdetails/userdetails.js";
...
function Datatable() {
  ...
  const getdata = async () => {
    await axios.get("http://127.0.0.1:8000/all_details", { headers: { Authorization: `Bearer ${session.user.token}` } }).then((response) => setLoadDatas(response.data.items))
  }

  useEffect(() => {
    getdata()
    dispatch(getpermissions(Loaddatas))
  }, []);
  ...
  const Update_info = (e) => {
    ...
    dispatch(UPDATEdetails({ infos, tkn }));
  ...
  const addPermission = (e) =>{
    ...
    const infos = {
      user_id: e.data.section_id,
      group_id: "Group id",
      section_id: e.data.section_id
    };

        //group send variables to Redux
        dispatch(AddDetail({ infos, tkn }));
